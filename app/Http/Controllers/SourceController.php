...
class SourceController extends Controller
{
  public function getTranslations($code): JsonResponse
  {
    ...
    return response()->json(collect($translationFiles)
      ...
      ->map(function ($value) {
        $str = Str::substr($value, 0, strlen($value) - strlen(LANG_EXT));
        if (Str::endsWith($str, '.lang')) {
          return Str::replace('.lang', '++lang', $str);
        }
        return $str;
      })
      ->flatten());
  ...
  {
    try {
      $fullPath = base_path() . '/source_' . $code . '/' . Str::replace("++", '.', $file) . LANG_EXT;
  ...
  {
    $fullPath = base_path() . '/source_translated/' . Str::replace("++", '.', $file) . LANG_EXT;
